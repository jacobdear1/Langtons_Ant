...
// use the typedef enum colour cell; to help define the cell_at(y,x)
enum colour *colour;

// takes the value of the enum colour and if it is black returns true, if not it returns false
#define cell_at(y, x) ( (*colour  == BLACK) ? true : false)
...
void start_visualisation(struct ant* ant) {
  setlocale(LC_ALL, "");
   ...
   if (ant->x > max_x){
      ant->x = 0;
      printf("Max x reached");
        // min_x?
    }

   if (ant->y > max_y){
      ant->y = 0;
      printf("Max y reached");
    }

    // if the ant goes below 0 (the bottom), then it goes to the rightmost row
   if (ant->x < 0){
      ant->x = max_x;
      printf("min x reached");
        // min_x?
    }

   if (ant->y < 0){
      ant->y = max_y;
      printf("Min y reached");
    }
}
