...
void turn_left(struct ant *ant) {
    // changes the ant's direction to turn 90 degrees clockwise, e.g. if the direction is left, 
    // then turning left would cause the ant to point down;
...
// need to change the state, once it has visited the state i 
// it changes to state i+1;

// don't want to access all rules, only the one for the 
// specific state
void apply_rule_general(enum gen_cell *gen_cell, struct ant *ant, struct rule *rule) {
    // printf("rule %c",*rule->rules);
    
    // puts the rule into an array and then find out the position 
    // of the colour and then apply the rule based off this? 
    // -> array is equal to the strlen (rule), so knows how 
    // many states it can give?
    // creates an array that has a length of all the rules in, 
    // and adds each one individually
    ...
    if (rule->rules[rule_val] == ('L')) {
        turn_left(ant);
        // change the state of this square -> change this in visualiser function? 
        // as this is what will corrrepsond to the symbols
        *gen_cell = *gen_cell + 1;
